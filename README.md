...
AI-Hub 실제 GPS 데이터를 추가하고 raw GPS에서 120초 feature를 만드는 방식으로 통일함.

Train, Validation, Test는 UID가 겹치지 않도록 나눔.

sampling cadence가 달라져도 feature가 무너지지 않는지 stress test를 진행함.

버스와 철도 reference가 잘못 연결되는 문제를 고치고 Transit Context를 보조 정보로 연결함.

Mock GPS Replay를 전체 Integration Pipeline에 연결함.

Movement, Temporal, Transit, Final 결과와 CO2, Reward를 한 화면에서 확인할 수 있게 만듦.
...
여정 계획에서 집과 직장의 실제 주소, 추천 경로, 예상 시간과 탄소 배출량을 확인함.

여정 시작 화면에서 출발지와 목적지를 확인하고 캐릭터가 사뿐사뿐 걷는 화면을 보여줌.

이동 중에는 현재 위치를 지도에서 따라가고 현재 이동수단과 이동 거리, 시간을 표시함.
...
bike와 bus는 walk나 rail보다 성능이 낮아서 실제 사용자 데이터로 추가 검증이 필요함.

현재 서울 학습 데이터는 따로 없고 AI-Hub와 GeoLife를 기준으로 만든 상태임.
