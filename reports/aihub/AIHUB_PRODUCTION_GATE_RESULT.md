...
| Validation Accuracy >= 0.70 | PASS (0.7235) | `hist_120_agg_rebuilt_metrics.json` |
| Test Accuracy >= 0.70 | PASS (0.7027) | `hist_120_agg_rebuilt_metrics.json` |
| Validation Macro F1 >= 0.65 | PASS (0.7276) | `hist_120_agg_rebuilt_metrics.json` |
| Test Macro F1 >= 0.65 | PASS (0.6914) | `hist_120_agg_rebuilt_metrics.json` |
...
HistGradientBoostingClassifier, seed 2021, 120초 aggregate window, 21개 AI-Hub feature, class weighting 없음. 기존 `geolife_hardened_120s_purity_090.joblib`는 rollback artifact로 유지합니다. 모델 파일은 저장소 정책상 Git에 넣지 않으며 `scripts/rebuild_aihub_production.ps1`로 재생성합니다.
