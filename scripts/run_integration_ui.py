...
# The consumer prototype below is the visual source-of-truth implementation. It keeps
# the existing API contract and replaces only the presentation layer.
MOBILE_APP_HTML = r'''<!doctype html>
<html lang="ko"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1,viewport-fit=cover"><title>Canopy</title>
<link rel="stylesheet" href="https://unpkg.com/leaflet@1.9.4/dist/leaflet.css" crossorigin>
<script src="https://unpkg.com/lucide@0.468.0"></script>
<style>
:root{--ink:#132d27;--muted:#6d8279;--green:#11845e;--mint:#e9f6ef;--line:#e1ebe5;--orange:#f36f4e;--blue:#2677ed}*{box-sizing:border-box}body{margin:0;background:#e6ece9;color:var(--ink);font-family:-apple-system,BlinkMacSystemFont,"SF Pro Display","Segoe UI",sans-serif}#phone{width:390px;max-width:100vw;height:844px;max-height:100vh;margin:0 auto;background:#fff;position:relative;overflow:hidden;box-shadow:0 20px 70px #193b2c3b}.screen{position:absolute;inset:0;display:none;background:#fff;animation:in .24s ease-out}.screen.active{display:block}@keyframes in{from{opacity:0;transform:translateY(8px)}to{opacity:1;transform:none}}.top{height:72px;padding:26px 20px 12px;display:flex;justify-content:space-between;align-items:center;position:absolute;inset:0 0 auto;z-index:10;background:linear-gradient(#fff 60%,#fff0);pointer-events:none}.top>*{pointer-events:auto}.brand{font-size:19px;font-weight:800;letter-spacing:.12em;color:#0e7d5a;display:flex;gap:8px;align-items:center}.brand i{width:20px;height:20px}.icon-btn{border:0;border-radius:50%;width:34px;height:34px;background:#f5f8f6;color:var(--ink);display:grid;place-items:center}.content{padding:76px 16px 92px;height:100%;overflow:auto}.eyebrow{font-size:11px;font-weight:800;letter-spacing:.13em;color:#628176;text-transform:uppercase}.title{font-size:27px;line-height:1.18;margin:8px 0 16px;font-weight:800}.sub{font-size:13px;color:var(--muted);line-height:1.5}.card{background:#fff;border:1px solid var(--line);border-radius:20px;padding:16px;box-shadow:0 7px 22px #183a2b10;margin:12px 0}.hero-card{background:linear-gradient(140deg,#f1fbf5,#fff);border:0;min-height:145px;position:relative;overflow:hidden}.hero-card img{position:absolute;width:150px;height:150px;object-fit:contain;right:-7px;bottom:-13px}.hero-number{font-size:37px;font-weight:800;margin:7px 0;color:#123f33}.hero-number span{font-size:18px}.goal{margin-top:15px}.progress{height:8px;background:#dfece5;border-radius:8px;overflow:hidden}.progress b{display:block;width:62%;height:100%;background:var(--green);border-radius:8px}.metric-row{display:flex;gap:9px;margin:12px 0}.metric{flex:1;background:#f3f7f4;border-radius:13px;padding:12px 10px}.metric small{display:block;color:var(--muted);font-size:10px}.metric strong{display:block;font-size:18px;margin-top:5px}.route-head{display:flex;justify-content:space-between;align-items:center;margin-bottom:11px}.route-head strong{font-size:19px}.pill{background:#edf3ef;padding:8px 10px;border-radius:16px;font-size:11px;font-weight:700}.choice-tabs{display:flex;gap:7px;overflow:auto;margin:8px 0 12px}.choice-tabs button{white-space:nowrap;border:0;background:#f2f6f3;color:#71847b;border-radius:18px;padding:9px 13px;font-size:12px}.choice-tabs button.selected{background:var(--green);color:#fff}.route-choice{border:1.5px solid #dfeae4;border-radius:16px;padding:13px;margin:9px 0}.route-choice.recommended{border-color:var(--green);box-shadow:0 4px 13px #11845e18}.route-choice header{position:static;height:auto;padding:0;background:none;display:flex;font-size:12px;justify-content:space-between}.route-choice .modes{display:flex;align-items:center;gap:8px;margin:15px 0 9px;color:#52665d}.route-choice .modes i{color:var(--green);width:21px}.route-choice .bottom{display:flex;justify-content:space-between;font-size:12px}.tag{font-size:10px;border-radius:9px;padding:3px 7px;background:#ddf4e8;color:#0c7954;font-weight:800}.tag.orange{background:#fff0e9;color:#ed6f4b}.btn{width:100%;border:0;border-radius:14px;padding:15px;background:var(--green);color:white;font-size:15px;font-weight:800;box-shadow:0 6px 15px #11845e38;transition:.2s}.btn:active{transform:scale(.97)}.map{position:absolute;inset:0 0 270px;background:#e9f1ed}.sheet{position:absolute;z-index:4;left:10px;right:10px;bottom:10px;border-radius:24px;background:#fffffff5;padding:17px 18px 16px;box-shadow:0 9px 29px #183a2b2b;backdrop-filter:blur(12px)}.sheet .btn{margin-top:11px}.start-hero{height:294px;padding-top:85px;text-align:center;background:linear-gradient(#fff,#f1f9f5)}.start-hero img{height:185px;max-width:230px;object-fit:contain}.start-hero h1{font-size:23px;margin:2px 0 6px}.start-hero p{margin:0;color:var(--muted);font-size:13px}.info-grid{display:grid;grid-template-columns:repeat(3,1fr);gap:8px}.info-grid .metric{text-align:center}.active-sheet{bottom:10px}.active-title{font-size:21px;font-weight:800;margin:6px 0}.live{display:inline-flex;align-items:center;gap:5px;background:#ffe9e8;color:#ea5550;border-radius:10px;padding:4px 7px;font-size:10px;font-weight:800}.live:before{content:'';width:6px;height:6px;border-radius:50%;background:#ee5450}.current-mode{display:flex;gap:12px;align-items:center;border-top:1px solid var(--line);padding-top:13px;margin-top:12px}.current-mode .mode-icon{width:42px;height:42px;border-radius:50%;background:#eaf2ff;color:var(--blue);display:grid;place-items:center}.current-mode strong{font-size:18px}.result-map{position:absolute;inset:0 0 440px}.complete-content{padding-top:74px}.complete-content .mascot{text-align:center;height:205px;position:relative}.complete-content .mascot img{height:205px;object-fit:contain}.complete-title{text-align:center;font-size:26px;font-weight:800;margin:2px 0 4px}.complete-sub{text-align:center;color:var(--muted);font-size:13px}.summary-card{margin-top:18px}.summary-card h3{margin:0 0 13px;font-size:14px}.summary-main{display:flex;justify-content:space-between;align-items:end}.summary-main strong{font-size:25px;color:var(--green)}.summary-main span{color:var(--muted);font-size:11px}.summary-bar{height:9px;background:#e5eee8;border-radius:8px;margin:13px 0 8px}.summary-bar b{display:block;width:81%;height:100%;border-radius:8px;background:var(--green)}.reward-card{background:#0a5d4b;color:#fff;border:0}.reward-card .eyebrow{color:#bce8cf}.reward-line{display:flex;align-items:center;justify-content:space-between;margin-top:8px}.reward-line strong{font-size:22px}.reward-line i{width:29px}.bottom-nav{position:absolute;left:9px;right:9px;bottom:9px;height:67px;border-radius:21px;background:#fffffff0;border:1px solid var(--line);display:flex;z-index:15;box-shadow:0 7px 20px #183a2b20}.bottom-nav button{flex:1;border:0;background:none;color:#94a39d;font-size:10px;font-weight:700;display:grid;place-items:center;gap:2px}.bottom-nav button i{width:19px}.bottom-nav button.active{color:var(--green)}.bottom-nav .center{margin:-16px auto 0;width:53px;height:53px;border-radius:50%;background:var(--green);color:#fff;box-shadow:0 5px 15px #11845e55}.profile{padding-top:82px}.profile-head{display:flex;align-items:center;gap:13px}.avatar{width:55px;height:55px;border-radius:50%;background:#dff3e8;color:var(--green);display:grid;place-items:center}.profile h2{margin:0;font-size:21px}.token-total{font-size:38px;font-weight:800;color:var(--green);margin:10px 0}.history{display:grid;gap:9px}.history-item{display:flex;justify-content:space-between;background:#f4f8f5;padding:12px;border-radius:12px;font-size:12px}.history-item strong{color:var(--green)}.developer{padding:80px 14px 18px;overflow:auto}.developer h2{margin-top:0}.developer label{display:block;font-size:12px;margin:9px 0}.developer select,.developer input,.developer textarea{width:100%;padding:9px;border:1px solid #ccd9d1;border-radius:9px;background:#fff}.developer button{padding:10px;border:1px solid #cbd8d0;border-radius:9px;background:#fff;margin:3px 0}.developer pre{white-space:pre-wrap;overflow:auto;background:#edf3ef;border-radius:9px;padding:9px;font-size:10px}.muted{color:var(--muted);font-size:11px}
</style></head><body><div id="phone"><header class="top"><div class="brand"><i data-lucide="leaf"></i>CANOPY</div><button class="icon-btn" onclick="openDeveloper()" aria-label="Developer"><i data-lucide="bell"></i></button></header>
<section id="home" class="screen active"><div class="content"><p class="sub">오늘도 지구를 위한 선택, 함께해요 🌍</p><div class="card hero-card"><div class="eyebrow">이번 달 탄소 절감량</div><div class="hero-number" id="monthReduction">0.0<span> kg</span></div><p class="sub">지난 달 대비 <b>25%</b> 증가했어요!</p><img src="/assets/canopy-ui/canopy-mascot.png" alt="Canopy mascot"></div><div class="card goal"><div class="eyebrow">나의 목표</div><p class="sub">이번 달 목표</p><strong>20kg 중 62% 달성</strong><div class="progress"><b></b></div><p class="sub" style="text-align:right">12.4 / 20kg</p></div><div class="eyebrow" style="margin:18px 4px 8px">주간 요약</div><div class="metric-row"><div class="metric"><small>이동 거리</small><strong id="homeDistance">0 km</strong></div><div class="metric"><small>이동 시간</small><strong>0h 0m</strong></div><div class="metric"><small>탄소 절감</small><strong id="homeReductionSmall">0 kg</strong></div></div><div class="card"><div class="route-head"><strong>현재 자주 직장</strong><i data-lucide="chevron-right"></i></div><p class="sub">주 5회 평균 2.3kg 절감</p></div></div><nav class="bottom-nav"><button class="active" data-tab="home" onclick="showScreen('home')"><i data-lucide="home"></i>홈</button><button data-tab="plan" onclick="showScreen('plan')"><i data-lucide="calendar-days"></i>여정 계획</button><button class="center" onclick="showScreen('start')"><i data-lucide="leaf"></i></button><button data-tab="complete" onclick="showScreen('complete')"><i data-lucide="award"></i>리워드</button><button data-tab="profile" onclick="showScreen('profile')"><i data-lucide="user-round"></i>마이페이지</button></nav></section>
<section id="plan" class="screen"><div class="content"><div class="route-head"><i data-lucide="arrow-left" onclick="showScreen('home')"></i><strong>여정 계획</strong><i data-lucide="sliders-horizontal"></i></div><div class="route-head"><strong>집 → 직장⌄</strong><span class="pill">08:30 출발</span></div><div class="choice-tabs"><button class="selected">추천</button><button>대중교통</button><button>자전거</button><button>도보</button><button>자동차</button></div><div class="card" style="background:#f4faf6;border:0"><div class="eyebrow">✦ 가장 친환경적인 경로</div><div class="route-choice recommended"><header><span class="tag">1순위 · 추천 경로</span><span>1시간 12분</span></header><div class="modes"><i data-lucide="person-standing"></i>→<i data-lucide="bus-front"></i>→<i data-lucide="person-standing"></i></div><div class="bottom"><b>₩2,500</b><span>탄소 배출 0.42kg <span class="tag">최소</span></span></div></div><div class="route-choice"><header><b>2순위 · 빠른 경로</b><span>53분</span></header><div class="modes"><i data-lucide="car-front"></i>→<i data-lucide="bus-front"></i>→<i data-lucide="person-standing"></i></div><div class="bottom"><b>₩4,800</b><span>탄소 배출 2.58kg <span class="tag orange">빠름</span></span></div></div><div class="route-choice"><header><b>3순위 · 자전거 경로</b><span>1시간 25분</span></header><div class="modes"><i data-lucide="bike"></i>→<i data-lucide="person-standing"></i></div><div class="bottom"><b>₩0</b><span>탄소 배출 0.61kg <span class="tag" style="background:#e5efff;color:#3478e8">보통</span></span></div></div></div><div class="card"><div class="eyebrow">KTDB 평균 대비</div><h3>1순위 경로 선택 시</h3><strong style="font-size:22px;color:var(--green)">2.16kg CO₂ 절감 <small>(81% 감소)</small></strong></div><button class="btn" onclick="showScreen('start')">이 경로로 시작하기</button></div><nav class="bottom-nav"><button onclick="showScreen('home')"><i data-lucide="home"></i>홈</button><button class="active"><i data-lucide="calendar-days"></i>여정 계획</button><button class="center" onclick="showScreen('start')"><i data-lucide="leaf"></i></button><button onclick="showScreen('complete')"><i data-lucide="award"></i>리워드</button><button onclick="showScreen('profile')"><i data-lucide="user-round"></i>마이페이지</button></nav></section>
<section id="start" class="screen"><div class="content" style="padding-top:0"><div class="start-hero"><img src="/assets/canopy-ui/canopy-mascot.png" alt="Canopy mascot"><h1>여정을 시작할까요?</h1><p>집 → 직장</p></div><div class="card"><div class="modes" style="justify-content:space-around;color:var(--muted)"><span><i data-lucide="person-standing"></i><small>걷기</small></span><span><i data-lucide="bike"></i><small>자전거</small></span><span><i data-lucide="bus-front"></i><small>대중교통</small></span><span><i data-lucide="car-front"></i><small>자동차</small></span><span><i data-lucide="person-standing"></i><small>도보</small></span></div></div><div class="card"><div class="eyebrow">이번 여정 목표</div><h3>탄소 배출 <strong style="color:var(--green)">0.42 kg</strong> 이하</h3><p class="sub">KTDB 평균 2.58 kg 대비</p><strong style="color:var(--green)">81% 절감 목표</strong></div><div class="eyebrow" style="margin:16px 3px 8px">예상 정보</div><div class="info-grid"><div class="metric"><small>예상 시간</small><strong>1시간 12분</strong></div><div class="metric"><small>예상 비용</small><strong>2,500원</strong></div><div class="metric"><small>이동 거리</small><strong id="planDistance">-</strong></div></div><button class="btn" onclick="startTrip()">여정 시작하기</button></div><nav class="bottom-nav"><button onclick="showScreen('home')"><i data-lucide="home"></i>홈</button><button onclick="showScreen('plan')"><i data-lucide="calendar-days"></i>여정 계획</button><button class="center"><i data-lucide="leaf"></i></button><button onclick="showScreen('complete')"><i data-lucide="award"></i>리워드</button><button onclick="showScreen('profile')"><i data-lucide="user-round"></i>마이페이지</button></nav></section>
<section id="active" class="screen"><div id="activeMap" class="map"></div><div class="sheet active-sheet"><div style="display:flex;justify-content:space-between;align-items:center"><div class="eyebrow">여정 진행 중 <span class="live">LIVE</span></div><i data-lucide="more-vertical"></i></div><div class="route-head" style="margin:13px 0 7px"><strong>집 → 직장</strong><span id="activeClock" class="pill">00:00</span></div><div class="sub" id="activeRouteName">출발지 → 도착지</div><div class="current-mode"><div class="mode-icon"><i data-lucide="bus-front"></i></div><div><small class="sub">현재 이동 수단</small><div id="activeStatus" class="active-title">이동 패턴을 확인하고 있어요</div><div id="activeSub" class="sub">120초 Window 처리 중</div></div></div><div class="metric-row"><div class="metric"><small>이동 시간</small><strong id="activeTime">00분</strong></div><div class="metric"><small>이동 거리</small><strong id="activeDistance">0.0 km</strong></div><div class="metric"><small>이동 비용</small><strong>-</strong></div></div><button class="btn" style="background:#fff;color:var(--orange);border:1px solid #f2d7d0" onclick="finishTrip()">이동 종료</button></div></section>
<section id="complete" class="screen"><div id="resultMap" class="result-map"></div><div class="content complete-content"><div class="mascot"><img src="/assets/canopy-ui/canopy-mascot.png" alt="Canopy mascot"></div><div class="complete-title">여정 완료!</div><div class="complete-sub">잘했어요! 지구를 위한 멋진 선택이에요 🌱</div><div class="card summary-card"><h3>이번 여정 요약</h3><div class="summary-main"><div><span>탄소 배출</span><strong id="resultActual">-</strong></div><div><span>KTDB 평균 대비</span><strong id="resultReduction">-</strong></div></div><div class="summary-bar"><b id="resultBar"></b></div><div class="sub" style="text-align:right">목표 0.42 kg</div><div class="metric-row"><div class="metric"><small>이동 시간</small><strong id="resultTime">-</strong></div><div class="metric"><small>이동 거리</small><strong id="resultDistance">-</strong></div><div class="metric"><small>이동 비용</small><strong>-</strong></div></div><p class="sub">감지된 이동수단: <b id="resultMode">-</b></p></div><div class="card"><div class="eyebrow">누적 절감 효과</div><strong style="font-size:22px;color:var(--green)" id="cumulativeReduction">0 kg CO₂</strong><p class="sub">30년생 소나무 1.5그루 심은 효과</p></div><div class="card reward-card"><div class="eyebrow">획득한 리워드</div><div class="reward-line"><strong id="resultToken">+0 Token</strong><i data-lucide="arrow-right"></i></div><div class="sub" style="color:#bce8cf">오늘의 챌린지 보너스!</div></div></div><nav class="bottom-nav"><button onclick="showScreen('home')"><i data-lucide="home"></i>홈</button><button onclick="showScreen('plan')"><i data-lucide="calendar-days"></i>여정 계획</button><button class="center"><i data-lucide="leaf"></i></button><button class="active"><i data-lucide="award"></i>리워드</button><button onclick="showScreen('profile')"><i data-lucide="user-round"></i>마이페이지</button></nav></section>
<section id="profile" class="screen"><div class="content profile"><div class="profile-head"><div class="avatar"><i data-lucide="user-round"></i></div><div><h2>마이페이지</h2><p class="sub">Canopy와 함께 만든 이동 기록</p></div></div><div class="card"><div class="eyebrow">현재 보유 Token</div><div class="token-total" id="profileToken">0 Token</div><p class="sub">탄소를 줄일 때마다 Token이 쌓여요.</p></div><div class="eyebrow" style="margin:19px 4px 8px">Token 받은 내역</div><div class="card history" id="profileHistory"><p class="sub">아직 받은 Token이 없어요.</p></div><div class="card"><div class="eyebrow">나의 이동 설정</div><div class="metric-row"><div class="metric"><small>집</small><strong>영등포구</strong></div><div class="metric"><small>직장</small><strong>종로구</strong></div></div></div></div><nav class="bottom-nav"><button onclick="showScreen('home')"><i data-lucide="home"></i>홈</button><button onclick="showScreen('plan')"><i data-lucide="calendar-days"></i>여정 계획</button><button class="center"><i data-lucide="leaf"></i></button><button onclick="showScreen('complete')"><i data-lucide="award"></i>리워드</button><button class="active"><i data-lucide="user-round"></i>마이페이지</button></nav></section>
<section id="developer" class="screen developer"><h2>Developer Mode</h2><label>Fixture<select id="fixture"></select></label><label>Replay speed<select id="speed"><option>instant</option><option>1</option><option>5</option><option>10</option><option>30</option></select></label><textarea id="expectedInput" rows="3" placeholder="KTDB MODEL_FEATURES JSON (선택)"></textarea><button onclick="startDeveloper()">기존 Fixture 실행</button><h3>AI-Hub Real GPS Replay</h3><p class="muted">Test UID만 선택합니다. Ground Truth는 inference 입력으로 보내지 않습니다.</p><label>Test trajectory<select id="aihubReplay"></select></label><label>AI-Hub dataset root<input id="aihubRoot" placeholder="C:\\Users\\user\\Downloads\\186.교통수단판별 데이터\\01-1.정식개방데이터"></label><button onclick="runAIHubReplay()">AI-Hub 결과 확인</button><pre id="aihubResult">WAITING</pre><h3>Replay / Window / Pipeline</h3><pre id="replay">-</pre><pre id="windows">WAITING</pre><pre id="pipeline">WAITING</pre><button onclick="showScreen('home')">User Mode로 돌아가기</button></section>
</div><script src="https://unpkg.com/leaflet@1.9.4/dist/leaflet.js" crossorigin></script><script>
...
function showScreen(name){currentScreen=name;document.querySelectorAll('.screen').forEach(x=>x.classList.toggle('active',x.id===name));document.querySelectorAll('.bottom-nav button').forEach(b=>b.classList.toggle('active',b.getAttribute('onclick')?.includes("'"+name+"'")));if(name==='profile')renderProfile();Object.values(maps).forEach(m=>setTimeout(()=>m.invalidateSize(),40));}
function openDeveloper(){showScreen('developer')}function setupMap(id){if(!window.L)return;const m=L.map(id,{zoomControl:false}).setView([37.55,126.95],12);L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png',{maxZoom:19,attribution:'© OpenStreetMap'}).addTo(m);maps[id]=m;layers[id]={path:L.polyline([],{color:'#2779ed',weight:5}).addTo(m),points:L.layerGroup().addTo(m)}}
function drawMaps(events=[]){const pts=events.filter(e=>e.accepted&&e.latitude!==undefined).map(e=>[+e.latitude,+e.longitude]);Object.entries(maps).forEach(([id,m])=>{layers[id].path.setLatLngs(pts);layers[id].points.clearLayers();if(pts.length){L.circleMarker(pts.at(-1),{radius:8,color:'#176144',fillColor:'#45bd88',fillOpacity:1}).addTo(layers[id].points);m.fitBounds(pts,{padding:[26,26],maxZoom:15})}else if(route.origin&&route.destination)m.fitBounds([[route.origin.latitude,route.origin.longitude],[route.destination.latitude,route.destination.longitude]],{padding:[30,30],maxZoom:14})})}
function renderBaseline(data){if(data?.status!=='READY')return;const p=data.probabilities||{};const label={walk:'도보',bike:'자전거',car:'자동차',bus:'버스',rail:'철도'};document.getElementById('planDistance').textContent=route.distance_km?route.distance_km.toFixed(1)+' km':'-';}
function modeText(mode){return {walk:'걷는 중이에요',bike:'자전거로 이동 중이에요',car:'자동차로 이동 중이에요',bus:'버스를 타고 있어요',rail:'지하철/철도로 이동 중이에요'}[mode]||'이동 패턴을 확인하고 있어요'}
function updateStatus(s){drawMaps(s.events||[]);const p=s.pipeline||{},w=[...(s.window_predictions||[])].reverse().find(x=>x.status==='READY');document.getElementById('activeDistance').textContent=Number(s.live_distance_km||0).toFixed(2)+' km';if(s.events?.length){const sec=(new Date(s.events.at(-1).timestamp)-new Date(s.events[0].timestamp))/1000;document.getElementById('activeTime').textContent=Math.floor(sec/60)+'분';document.getElementById('activeClock').textContent=formatTime(sec)}if(w){document.getElementById('activeStatus').textContent=modeText(w.predicted_mode);document.getElementById('activeSub').textContent='confidence '+(Number(w.confidence||0)*100).toFixed(0)+'%'}if(p.status==='PASS'){lastResult=p;document.getElementById('resultDistance').textContent=Number(p.distance_km||0).toFixed(2)+' km';document.getElementById('resultExpected').textContent=Number(p.co2?.expected_co2e_g||0).toFixed(1)+' g';document.getElementById('resultActual').textContent=(Number(p.co2?.actual_co2e_g||0)/1000).toFixed(2)+' kg';document.getElementById('resultReduction').textContent=(Number(p.co2?.reduction_co2e_g||0)/1000).toFixed(2)+' kg 절감';document.getElementById('resultMode').textContent=(p.actual_behaviour?.mode_sequence||[]).filter((x,i,a)=>i===0||x!==a[i-1]).join(' → ')||'-';document.getElementById('resultTime').textContent=s.events?.length?formatTime((new Date(s.events.at(-1).timestamp)-new Date(s.events[0].timestamp))/1000):'-';const reduction=Math.max(0,Number(p.co2?.reduction_co2e_g||0));const earned=Math.floor(reduction/10);document.getElementById('resultToken').textContent='+'+earned+' Token';document.getElementById('cumulativeReduction').textContent=(reduction/1000).toFixed(2)+' kg CO₂';tokenBalance=Math.max(tokenBalance,tokenBalance+earned);localStorage.setItem('canopyTokenBalance',String(tokenBalance));if(currentScreen==='active')showScreen('complete')}}
...
function renderProfile(){document.getElementById('profileToken').textContent=tokenBalance+' Token';let h=[];try{h=JSON.parse(localStorage.getItem('canopyTokenHistory')||'[]')}catch{}document.getElementById('profileHistory').innerHTML=h.length?h.map(x=>'<div class="history-item"><span>'+x.date+'</span><strong>+'+x.earned+' Token</strong></div>').join(''):'<p class="sub">아직 받은 Token이 없어요.</p>'}
async function startTrip(){showScreen('active');await api('/api/start',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({fixture:'mock/canopy_iphone_mock_yeongdeungpo_to_microsoft.csv',speed:'30',view_mode:'user'})})}async function finishTrip(){await api('/api/stop',{method:'POST'})}async function startDeveloper(){showScreen('active');let e=null;const t=document.getElementById('expectedInput').value.trim();if(t)e=JSON.parse(t);await api('/api/start',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({fixture:document.getElementById('fixture').value,speed:document.getElementById('speed').value,expected_features:e,view_mode:'developer'})})}async function runAIHubReplay(){const result=document.getElementById('aihubResult');result.textContent='REPLAYING...';result.textContent=JSON.stringify(await api('/api/aihub/replay',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({replay_id:document.getElementById('aihubReplay').value,source_root:document.getElementById('aihubRoot').value,speed:document.getElementById('speed').value})}),null,2)}
async function init(){const r=await api('/api/route');Object.assign(route,r);const b=await api('/api/baseline');renderBaseline(b);const f=await api('/api/fixtures');document.getElementById('fixture').innerHTML=f.map(x=>'<option>'+x+'</option>').join('');const m=await api('/api/aihub/manifest');document.getElementById('aihubReplay').innerHTML=(m.trajectories||[]).map(x=>'<option value="'+x.replay_id+'">'+x.replay_id+' · '+x.ground_truth+'</option>').join('');setupMap('activeMap');setupMap('resultMap');drawMaps();lucide.createIcons();setInterval(async()=>{const s=await api('/api/status');updateStatus(s);document.getElementById('replay').textContent=JSON.stringify(s.replay,null,2);document.getElementById('windows').textContent=JSON.stringify(s.window_predictions||'WAITING',null,2);document.getElementById('pipeline').textContent=JSON.stringify(s.pipeline||'WAITING',null,2)},700)}init();
</script></body></html>'''

# Keep the expected CO2 value visible without changing the result card structure.
MOBILE_APP_HTML = MOBILE_APP_HTML.replace(
    '<div class="summary-main">',
    '<div class="sub">Expected CO₂ <b id="resultExpected">-</b></div><div class="summary-main">',
    1,
)
MOBILE_APP_HTML = MOBILE_APP_HTML.replace(
    '<body><div id="phone">',
